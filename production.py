...
def backward_chain(rules, hypothesis, verbose=False):
    ...
    """
    bindings = []  #A list to store the bindings
    facts = set() #A set to store the facts to be checked
    facts.add(hypothesis)
    final_facts = set() #A set to store the facts that appear in the final output
    final_facts.add(hypothesis)
    """
    A recursive function to check whether the nodes are related or not
    It does so by receiving as an input the initial hypothesis, treating it as a consequential,
    seeing if it matches any other consequentials, populating the antecedents of that matching rule
    with the matches, and adding them to a new set, then we check if the new facts are consequentials
    """
    def recursive(facts):
        new_facts = set()
        for fact in facts:
            for rule in rules:
                for action in rule._action:
                    match_result = match(action, fact)

                    if match_result:

                        if isinstance(rule._conditional, AND):
                            for r in rule._conditional:
                                new_facts.add(populate(r, match_result))
                                final_facts.add(populate(r, match_result))
                        elif isinstance(rule._conditional, OR):
                            #if the statement is united through or, we can't be sure if
                            #the tourist posseses only one or both of the qualities
                            or_statement = ' EITHER/OR '.join(populate(r, match_result) for r in rule._conditional)
                            final_facts.add(or_statement)
                            for r in rule._conditional:
                                new_facts.add(populate(r, match_result))


                        bindings.append(match_result)
                        if verbose:
                            print("Matched:", action, "with", hypothesis)

        # Check if new facts were added
        if new_facts:
            # Recur with the new facts
            recursive(new_facts)
    ...
    def apply(self, rules, apply_only_one=False, verbose=False):
        ...
        for k in bindings:
            for a in self._action:
                ...
                """
                I added to the apply() method in order to be able to remove the rules that participated in 
                the discovery of a new rule, for this I created new bindings between the antecedent rules and 
                the existing rules, then, I populated them with the character name and created a set for deletion
                """
                bind = RuleExpression().test_term_matches(self.antecedent(), new_rules)
                for c in bind:
                    for b in self.antecedent():
                        to_delete.add(populate(b, c))
        ...
        return tuple(sorted(new_rules-to_delete)) # Uniquify and sort the
                                        # output list
...
def starts_with_and_or(expression):
    return re.match(r'^\s*(AND|OR)\(', expression) is not None
